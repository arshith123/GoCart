...
const SideBar = ({ collapsed, mobileOpen, setCollapsed }) => {
  const navItems = [
    {
      icon: <LayoutDashboard color="#e0e7ff" strokeWidth={1.5} />,
      label: "Dashboard",
      link: "/dashboard",
    },
    {
      ...
      subMenu: [
        ...
        { label: "Categories", link: "/category-list" },
        { label: "Brands", link: "/brand-list" },
        { label: "UOM", link: "/uom-list" },
      ],
    },
    {
      icon: <Users strokeWidth={1.5} color="#e0e7ff" />,
      label: "User Management",
      link: "/user-managment",
    },
    { icon: <House strokeWidth={1.5} color="#e0e7ff" />, label: "Inventory" },
    {
      icon: <Landmark strokeWidth={1.5} color="#e0e7ff" />,
      label: "Accounts",
      subMenu: [
        { label: "Tax", link: "/tax-list" },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700 h-20">
        ...
        >
          {collapsed ? (
            <ListIndentIncrease color="#e0e7ff" size={20} />
          ) : (
            <ListIndentDecrease color="#e0e7ff" size={20} />
          )}
        </button>
      </div>
        ...
        {navItems.map((item, idx) => {
          ...
          return (
            <li
              ...
              className="relative"
              onMouseEnter={() =>
                collapsed && item.subMenu && setHoveredIndex(idx)
              }
              onMouseLeave={() =>
                collapsed && item.subMenu && setHoveredIndex(null)
              }
            >
              {/* Main nav item */}
              <div
              ...
              >
                <span
                  className={`w-6 h-6 flex justify-center items-center transition-transform ${active ? "scale-110" : "group-hover:scale-110"}`}
                >
                  {item.icon}
                </span>
                {!collapsed && (
                  <span
                    className={`font-medium text-sm font-sans transition-colors flex-1 ${active ? "text-white" : "text-slate-200 group-hover:text-white"}`}
                  >
                    {item.label}
                  </span>
                )}
