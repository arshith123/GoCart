...
const AdminLogin = () => {
  ...
  // useMutation is used for POST/PUT/DELETE — login is a POST request
  const loginMutation = useMutation({
    mutationFn: ({ email, password }) => authService.login(email, password),
    onSuccess: (data) => {
      // data.data contains { user, token } based on backend RESPONSE helper
  ...
  return (
    <div className="flex h-screen">
      ...
      <div
        ...
        style={{
          background: "linear-gradient(135deg, #5B21B6, #9333EA, #7C3AED)",
        }}
      >
        {/* Decorative circles */}
      ...
      <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-6">
        <div className="bg-white py-10 px-8 rounded-2xl shadow-2xl w-full max-w-md border border-gray-100">
          ...
          <div className="lg:hidden flex items-center justify-center gap-2 mb-6">
            ...
            <h1 className="text-3xl font-bold font-heading bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent">
              GoCart
            </h1>
          </div>

          <h1 className="text-3xl font-bold mb-2 text-center font-heading text-gray-800">
            Welcome Back
          </h1>
          ...
          <form onSubmit={handleLogin} autoComplete="off">
            <div className="flex flex-col gap-5">
              ...
              <button
                ...
                className="w-full px-10 py-4 text-white font-semibold rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 mt-2 disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2"
              >
